...
class UADETRAC(IMDB):
	...
	self.year = 2017
        ...
        super(UADETRAC, self).__init__('UADETRAC', image_set, root_path, devkit_path, result_path)  # set self.name

        #self.year = year
        self.root_path = root_path
        self.devkit_path = devkit_path
        self.data_path = os.path.join(devkit_path, 'UADETRAC')
    ...
    def image_path_from_index(self, index):
        ...
        """
        image_files_folder = os.path.join(self.data_path, 'TrainSequences', index)
        assert os.path.exists(image_files_folder), 'Path does not exist: {}'.format(image_files_folder)
	...
	filename = os.path.join(self.data_path, 'TrainAnnotationsXML', index + '.xml')
